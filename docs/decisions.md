...
## 107. Rules the review rounds left behind (§61–§104)

Between 2026-09-19 and 2026-09-21 the tree went through some forty review
rounds. Their round-by-round journals (§61–§90, §92–§102, §104) live in
the private repo; the durable rules they produced are these.

**The helper's boundary**

- Malformed input never reaches xkbcommon: an interior NUL in any field,
  or a keymap declaring a keycode above 4096, is refused at every compile
  door.
- A custom `kb_file` is opened first (non-blocking), checked on the
  descriptor (regular file, ≤ 2 MiB) and read once; that one bounded
  snapshot feeds both validation and install.
- The group ceiling comes from the *compiled* map, never the declared
  layout list (the evdev rules resolve at most four layouts).
- The handshake is a gate: nothing but a matching `hello` runs before
  negotiation. The handshake window, the hold cap and the per-line frame
  cap are enforced on every iteration regardless of traffic, and every
...
- Every command sent occupies one `ChordAcks` slot and every reply pops
  the oldest. A chord's verdict is its own final line's ack, poisoned by
  any `err` inside its region.
- A handoff completes when the counterpart answers, not when the write
  returns. Residual: an ack means the compositor has the events, not that
  the client consumed the paste; only a paste-confirmed protocol reply
  could say more, and none exists.
- A verdict carries the sequence it was armed for; a verdict for another
  sequence lands stale.
- A killed process is never re-armed inline: its exit re-drives through
...
- Saves are private at creation (umask 077, temp + rename), queued per
  target, and a failed save is reported per path.

**Tests and gates**

- Defects live where two operations overlap, not inside one state
  machine: the VM integration suite pins the overlap contracts on the
  live socket.
- A pin drives the real function and derives the expected value
  independently; a test that restates its formula is a defect.
- Gates enumerate the filesystem, and an empty enumeration fails.
