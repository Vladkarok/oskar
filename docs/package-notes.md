...
- `/usr/bin/oskar` — the lifecycle command (setup / upgrade /
  status / teardown)
- `/usr/lib/oskar/oskar-daemon` — the input helper
- `/usr/lib/systemd/user/oskar.service` — the user unit (already
  points at the packaged helper)
- `/usr/share/oskar/plugin/` — the runtime QML/JS/assets payload

## The lifecycle

- **`oskar setup`** — preflight dependencies; register exactly
  `/usr/share/oskar/plugin` under the stable plugin id
  `io.github.vladkarok.oskar`; rescan; enable the plugin through the
...
- **`oskar status`** — payload, registration target, plugin
...
the user unit, and links `bin/oskar` into `~/.local/bin`, where it
...
- [ ] F7 hygiene: after `pacman -R oskar`, no unowned files may
      remain under /usr/share/oskar (the 0.1.0-2 pkgrel once
      left LanguageControl.js/HoldColumn.js behind — file-list drift
      between pkgrels; verify with `pacman -Ql` vs the tree before
      tagging — `oskar doctor` names any strays in its
      version-drift check).
