...
Item {
    ...
    // The helper connection's transport: the socket object in its loader,
    // the one rebuild, the hello and reconnect timers, the socket-file
    // check and the quiescent ping. What a line MEANS is the keyboard's —
    // every reply reaches its dispatch through `lineReceived`, and every
    // write, either direction, crosses the keyboard's one choke point
    ...
    // never the repair timer's re-hello of a live one.
    ...
    // tear a live object graph down safely. Shared by onError, the
    // watchdog's "rebuild" answer and the path check — one place that
    // knows a fresh socket object is the cure. The keyboard's own state
    // resets run synchronously through `rebuilt()` before the teardown
    // is queued.
    function rebuild() {
    ...
    // against graphical-session.target, not against the shell, so the
    // panel's first connection attempt can find no socket. Quickshell's
    // Socket never recovers from that — a failed connect leaves its
    // internal QLocalSocket in place, and setConnected(true) only dials
    // when that object is gone, with nothing but a successful connection
    // ever clearing it — so the whole socket is rebuilt whenever the
    // helper's socket file exists and the helper has not answered hello
    // yet. A helper that dies later needs none of this: the disconnected
    // path clears the object and the pending targetConnected redials on
    // its own.
    Loader {
        id: helperLoader
        active: true
        ...
        Socket {
            ...
            onConnectionStateChanged: {
                if (connected) {
                    ...
                    // helper accepts commands before the compositor keymap
                    // has been forwarded to its virtual keyboard, and would
                    // drop every key. hello therefore goes out on a short
                    // delay after the flip. The flip is also what the hello
                    // reply's reset keys off: only a genuinely new
                    // connection released the old one's holds.
                    ...
                    // else the disconnect needs is keyboard state, handled
                    // via connectionDropped().
                    link.helloInFlight = false
            ...
            // without ever flipping `connected` — the property can keep
            // reading true minutes after QLocalSocket::PeerClosedError —
            // which leaves inputReady stuck at true: a ready-looking
            // keyboard that cannot type. An error arriving on a socket that
            // still reads connected is therefore treated as the drop the
            // state change failed to report: enter the disconnected state
            // and rebuild the socket object, so the gated repair timer owns
            // the redial. A failed dial reports with connected false and
            // no-ops here, so this cannot loop.
            onError: {
                if (!connected) return
                link.rebuild()
            }
    ...
    Timer {
        id: reconnectTimer
        // Adaptive, and never stopped: a socket that lies `connected` on a
        // peer-closed transport must still be asked something, or every
        // keystroke writes into the void until an unrelated event or a
        // shell restart. Healthy, the tick is slow and asks with ping
        // (below); unhealthy or with a probe still outstanding, it keeps
        // the repair cadence so a wedge is noticed, judged and acted on
        // inside ~7 s.
        // Also while a configure is outstanding: a socket that stays
        // `connected` but never answers `configured` would leave the panel
        // ready-looking, drawing the acked group while the helper types
        // the queued one, with no tick to notice. A configure is answered
        // in well under the fast interval, so an entry still outstanding
        ...
        running: true
        onTriggered: () => {
            // The decision is SocketWatch's pure table. A probe outstanding
            // past its fair window rebuilds the socket object whatever
            // `connected` claims; a live helper answers in well under a
            // second, so only a socket that cannot deliver ever sees the
            // window expire. The rebuild path is also the silent-dial cure:
            // a fresh socket that connects without ever signalling gets a
            // hello on the next tick, and the tick after that treats its
