...
## omarchy-shell-emoji-osk.patch — withdrawn 2026-09-09, file deleted

This patch carried the opt-in OSK cooperation for the shell emoji overlay
(`omarchy.emojis`): an `{"osk":true, output, workArea, band}` summon
payload, vendored `PickerFit.planPlacement` inside the overlay, band
subtraction from the input region, and the Exclusive-then-OnDemand focus
prime. Nothing summons it any more — ticket 24 replaced the external
picker with the panel's own emoji page, and the external app, when
wanted, is launched bare from that page's chip with no cooperation from
us (decisions §24). The patch itself is recoverable from this repo's
history; a machine still carrying it applied can restore stock files with
`sudo pacman -S omarchy-dev` (or `omarchy update`).
